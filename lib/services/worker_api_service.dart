...
class WorkerApiService {
  ...
  Future<List<String>> jobCategories() async =>
      ((_payload(await _api.get(ApiConstants.jobCategories)))['job_categories']
          ...
          .map((e) => e.toString())
          .toList();
  Future<WorkerProfileModel> profile() async => WorkerProfileModel.fromJson(
    _payload(await _api.get(ApiConstants.workerProfile)),
  );
  Future<WorkerProfileModel> updateProfile(Map<String, dynamic> values) async =>
      WorkerProfileModel.fromJson(
        _payload(await _api.patch(ApiConstants.workerProfile, values)),
      );
  Future<String?> uploadAvatar(File image) async => (await _api.multipart(
    ApiConstants.avatar,
    {},
    {'avatar': image},
  ))['avatar_url']?.toString();
  Future<bool> setAvailability(bool value) async =>
      (await _api.patch(ApiConstants.availability, {
        'available': value,
      }))['available'] ==
      true;
  ...
  Future<Map<String, dynamic>> jobs({
    ...
    ApiConstants.jobs,
    query: {...?filters, if (page != null) 'page': page},
  );
  Future<JobPageModel> fetchJobs({
    Map<String, dynamic>? filters,
    int? page,
  }) async => JobPageModel.fromJson(await jobs(filters: filters, page: page));
  ...
  Future<ApplicationPageModel> fetchApplications({
    String? status,
    int? page,
  }) async => ApplicationPageModel.fromJson(
    await applications(status: status, page: page),
  );
  ...
  Future<Map<String, dynamic>> submitKyc({
    ...
    required File panDoc,
    required File aadhaarDoc,
  }) => _api.multipart(
    ApiConstants.kyc,
  ...
  Future<List<NotificationModel>> fetchNotifications({int? page}) async {
    ...
    return jsonList(
      paginated['data'],
    ).map((e) => NotificationModel.fromJson(jsonMap(e))).toList();
  }
  ...
  Future<int> readNotification(String id) async =>
      ((await _api.post(ApiConstants.readNotification(id)))['unread'] as num?)
          ?.toInt() ??
      0;
  Future<int> readAllNotifications() async =>
      ((await _api.post(ApiConstants.readAllNotifications))['unread'] as num?)
          ?.toInt() ??
      0;
  ...
  Future<List<ReviewModel>> fetchReviews({int? page}) async {
    ...
    return jsonList(
      paginated['data'],
    ).map((e) => ReviewModel.fromJson(jsonMap(e))).toList();
  }

  Future<ReviewPageModel> fetchReviewPage({int? page}) async =>
      ReviewPageModel.fromJson(await reviews(page: page));
  Future<void> reviewEmployer(
    int applicationId,
    int rating, {
    String? comment,
  }) async {
    await _api.post(ApiConstants.reviewApplication(applicationId), {
      'rating': rating,
  ...
  Future<String> setLocale(String locale) async =>
      (await _api.post(ApiConstants.locale, {
        'locale': locale,
      }))['locale']?.toString() ??
      locale;
  Future<LocaleModel> updateLocale(String locale) async => LocaleModel.fromJson(
    await _api.post(ApiConstants.locale, {'locale': locale}),
  );
  ...
  Future<ResumeModel> uploadResume(File file) async => ResumeModel.fromJson(
    jsonMap(
      (await _api.multipart(ApiConstants.resume, {}, {
        'resume': file,
      }))['resume'],
    ),
  );
  ...
  Future<PreferencesModel> fetchPreferences() async =>
      PreferencesModel.fromJson(
        jsonMap((await _api.get(ApiConstants.preferences))['preferences']),
      );
  Future<PreferencesModel> updatePreferences(
    Map<String, dynamic> values,
  ) async => PreferencesModel.fromJson(
    jsonMap(
      (await _api.patch(ApiConstants.preferences, values))['preferences'],
    ),
  );
  Future<Map<String, dynamic>> support() => _api.get(ApiConstants.support);
  Future<Map<String, dynamic>> legal() => _api.get(ApiConstants.legal);
  Future<Map<String, dynamic>> legalDocument(String key) =>
      _api.get(ApiConstants.legalDocument(key));
  Future<ConversationPageModel> fetchConversations() async =>
      ConversationPageModel.fromJson(
        await _api.get(ApiConstants.conversations),
      );
  Future<ConversationModel> openConversation({
    ...
    jsonMap(
      (await _api.post(ApiConstants.conversations, {
  ...
  Future<MessageModel> sendMessage(int id, String body) async =>
      MessageModel.fromJson(
        jsonMap(
          (await _api.post(ApiConstants.conversationMessages(id), {
            'body': body.trim(),
          }))['message'],
        ),
      ).asMine();
  ...
  Future<List<SessionModel>> fetchSessions() async => jsonList(
    (await _api.get(ApiConstants.sessions))['sessions'],
  ).map((e) => SessionModel.fromJson(jsonMap(e))).toList();
