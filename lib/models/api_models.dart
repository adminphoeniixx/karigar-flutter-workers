...
class ReferenceData {
  ...
  factory ReferenceData.fromJson(Map<String, dynamic> j) {
    ...
    return ReferenceData(
      states: list('states'),
      skills: list('skills'),
      spokenLanguages: list('spoken_languages'),
...
String jobWageLabel(Json json) {
  ...
  String? amount(dynamic value) {
    ...
    final plain = number == number.roundToDouble()
        ...
        : number.toString();
    return plain.replaceAllMapped(RegExp(r'\B(?=(\d{3})+(?!\d))'), (_) => ',');
  }
...
class ApplicationModel {
  ...
  factory ApplicationModel.fromJson(Json json) => ApplicationModel(
    ...
    trackingSteps: jsonList(
      json['tracking_steps'],
    ).map((e) => TrackingStepModel.fromJson(jsonMap(e))).toList(),
    interview: json['interview'] is Map
        ? InterviewModel.fromJson(jsonMap(json['interview']))
        : null,
    offer: json['offer'] is Map
        ? OfferModel.fromJson(jsonMap(json['offer']))
        : null,
    canReview: jsonBool(json['can_review']),
  );
}
...
class InterviewModel {
  ...
  factory InterviewModel.fromJson(Json j) => InterviewModel(
    atLabel: j['at_label']?.toString() ?? '',
...
class OfferModel {
  ...
  factory OfferModel.fromJson(Json j) => OfferModel(
    wage: jsonDouble(j['wage']),
...
class ResumeModel {
  ...
  factory ResumeModel.fromJson(Json j) => ResumeModel(
    name: j['name']?.toString() ?? 'Resume.pdf',
    uploadedAgo: j['uploaded_ago']?.toString() ?? '',
    characters: jsonInt(j['characters']),
    maxCharacters: jsonInt(j['max_characters']),
  );
}
...
class PreferencesModel {
  ...
  factory PreferencesModel.fromJson(Json j) => PreferencesModel(
    theme: j['theme']?.toString() ?? 'system',
    jobAlerts: jsonBool(j['job_alerts']),
    messageAlerts: jsonBool(j['message_alerts']),
  );
}
...
class ConversationModel {
  ...
  factory ConversationModel.fromJson(Json j) => ConversationModel(
    id: jsonInt(j['id']),
    otherParty: jsonMap(j['other_party'])['name']?.toString() ?? 'Employer',
...
class MessageModel {
  ...
  factory MessageModel.fromJson(Json j, {int? currentUserId}) {
    ...
    final senderRole = (sender['role'] ?? j['sender_role'] ?? j['sender_type'])
        ?.toString()
        .toLowerCase();
    final senderId = jsonInt(j['sender_id'] ?? j['user_id'] ?? sender['id']);
...
class ConversationPageModel {
  ...
  factory ConversationPageModel.fromJson(Json j) => ConversationPageModel(
    conversations: jsonList(
      j['conversations'] ?? j['data'],
    ).map((e) => ConversationModel.fromJson(jsonMap(e))).toList(),
...
class SessionModel {
  ...
  final bool current;
  factory SessionModel.fromJson(Json j) => SessionModel(
    id: j['id']?.toString() ?? '',
    device: j['device']?.toString() ?? 'Unknown device',
    current: jsonBool(j['current']),
    lastUsedAgo: j['last_used_ago']?.toString() ?? '',
  );
}
...
class ApplicationPageModel {
  ...
  factory ApplicationPageModel.fromJson(Json json) => ApplicationPageModel(
    applications: jsonList(
      json['data'],
    ).map((e) => ApplicationModel.fromJson(jsonMap(e))).toList(),
    pagination: PaginationModel.fromJson(jsonMap(json['meta'])),
  );
}
...
class ReviewModel {
  ...
  factory ReviewModel.fromJson(Json json) => ReviewModel(
    ...
    createdAgo:
        json['created_ago']?.toString() ?? json['created_at']?.toString() ?? '',
    reviewer: EmployerModel.fromJson(
      jsonMap(json['reviewer'] ?? json['employer']),
    ),
    jobTitle: json['job'] is Map
        ? jsonMap(json['job'])['title']?.toString()
        : null,
  );
}
...
class DashboardStatsModel {
  ...
  factory DashboardStatsModel.fromJson(Json json) => DashboardStatsModel(
    availableJobs: jsonInt(json['available_jobs']),
    applications: jsonInt(json['applications']),
    savedJobs: jsonInt(json['saved_jobs']),
    kycStatusLabel: json['kyc_status_label']?.toString() ?? '',
    profileCompletion: jsonInt(json['profile_completion']),
